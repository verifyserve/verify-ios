...
import 'Pending_OwnerDocument_Fill.dart';
...
class Catid_Owner {
  ...
  factory Catid_Owner.FromJson(Map<String, dynamic>json){
    return Catid_Owner(id: json['id'],
        Name_: json['Name_'],
        Number: json['Number'],
...
class ApplyDocument_Owner extends StatefulWidget {
  const ApplyDocument_Owner({super.key});

  @override
  State<ApplyDocument_Owner> createState() => _ApplyDocument_OwnerState();
}

class _ApplyDocument_OwnerState extends State<ApplyDocument_Owner> {
  ...
  Future<List<Catid_Owner>> fetchData_Owner() async{
    var url=Uri.parse("https://verifyserve.social/WebService4.asmx/show_store_document_by_number_?number=${_OwnerNumber}");
    final responce=await http.get(url);
    if(responce.statusCode==200){
      List listresponce=json.decode(responce.body);
      return listresponce.map((data) => Catid_Owner.FromJson(data)).toList();
    }
    else{
                    ...
                    itemBuilder: (context, index) {
                      return GestureDetector(
                        onTap: (){
                          ...
                          context,
                          MaterialPageRoute(
                            builder: (context) => Owner_Document(id: '${snapshot.data![index].id.toString()}',),
                          ),
                        );
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        onPressed: () async {
          ...
          if (responce.body == '[{"logg":1}]') {
            ...
            fetchData_Owner();
            final result_Owner = await fetchData_Owner();

            Navigator.of(context).push(

              MaterialPageRoute(
                builder: (context) => Owner_AutoFill_Form(id: '${result_Owner.first.id.toString()}',),
              ),
            );
